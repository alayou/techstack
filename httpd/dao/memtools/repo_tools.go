...
const (
	...
	FieldValuePropose   = "ValuePropose"
	FieldQuickStart     = "QuickStart"
	FieldTechStack      = "TechStack"
	FieldCodeStructure  = "CodeStructure"
	FieldCodeRule       = "CodeRule"
	FieldMainAPI        = "MainAPI"
	FieldUsageScenarios = "UsageScenarios"
	FieldStrength       = "Strength"
	FieldWeakness       = "Weakness"
	FieldSuitFor        = "SuitFor"
	FieldNotSuitFor     = "NotSuitFor"
)
...
// updateRepoField 更新内存中 RepoTechAnalysis 结构的字段
...
func (u *updateRepoField) Call(ctx context.Context, input string) (out string, err error) {
	...
	v := reflect.ValueOf(u.result).Elem()
	f := v.FieldByName(u.field)
	if !f.IsValid() {
		return "error: field " + u.field + " does not exist", nil
	}
	if f.Kind() != reflect.String {
		return "error: field " + u.field + " is not a string type", nil
	}
	f.SetString(content)
...
// NewRepoTools 创建仓库技术分析的所有工具（读写）
func NewRepoTools(result *model.RepoTechAnalysis) []tools.Tool {
...
func NewReadRepoTools(result *model.RepoTechAnalysis) []tools.Tool {
...
func NewUpdateRepoTools(result *model.RepoTechAnalysis) []tools.Tool {
	toolsList := make([]tools.Tool, 0, len(repoFieldDefs))
