...
const _testGoMod = `module code.gitea.io/gitea
...
func TestParseGoMod(t *testing.T) {
	pkgs, err := ParseGoMod(_testGoMod)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, dep := range expectedDirectDeps {
		if !pkgNames[dep] {
			t.Errorf("Expected direct dependency not found: %s", dep)
		}
	}

	// 验证 require 块中的间接依赖
	expectedIndirectDeps := []string{
	...
	for _, dep := range expectedIndirectDeps {
		if !pkgNames[dep] {
			t.Errorf("Expected indirect dependency not found: %s", dep)
		}
	}

	// 验证 replace 指令中的包
	expectedReplaceDeps := []string{
	...
	for _, dep := range expectedReplaceDeps {
		if !pkgNames[dep] {
			t.Errorf("Expected replace dependency not found: %s", dep)
		}
	}

	// 验证不包含 module 声明
	if pkgNames["code.gitea.io/gitea"] {
		t.Error("Should not include module declaration")
	}

	// 验证不包含 go 版本声明
	if pkgNames["go"] {
		t.Error("Should not include go version")
	}

	t.Logf("Total packages found: %d", len(pkgs))
}

func TestParseGoModSimple(t *testing.T) {
	simpleGoMod := `module github.com/example/myapp
...
replace github.com/jaytaylor/html2text => github.com/Necoro/html2text v0.0.0-20220115212137-fc9d2f7edfb9
`

	pkgs, err := ParseGoMod(simpleGoMod)
	if err != nil {
		t.Fatal(err)
	}
	...
	// 验证直接依赖及其版本
	if pkg, ok := pkgMap["github.com/gin-gonic/gin"]; ok {
		if pkg.Version != "v1.9.1" {
			t.Errorf("Expected version v1.9.1 for github.com/gin-gonic/gin, got %s", pkg.Version)
		}
	} else {
		t.Error("Expected direct dependency github.com/gin-gonic/gin not found")
	}

	if pkg, ok := pkgMap["github.com/google/uuid"]; ok {
		if pkg.Version != "v1.4.0" {
			t.Errorf("Expected version v1.4.0 for github.com/google/uuid, got %s", pkg.Version)
		}
	} else {
		t.Error("Expected direct dependency github.com/google/uuid not found")
	}

	// 验证间接依赖及其版本
	if pkg, ok := pkgMap["github.com/bytedance/sonic"]; ok {
		if pkg.Version != "v1.9.1" {
			t.Errorf("Expected version v1.9.1 for github.com/bytedance/sonic, got %s", pkg.Version)
		}
	} else {
		t.Error("Expected indirect dependency github.com/bytedance/sonic not found")
	}

	// 验证 replace 指令及其版本
	if pkg, ok := pkgMap["github.com/jaytaylor/html2text"]; ok {
		if pkg.Version == "" {
			t.Error("Expected version for replace dependency github.com/jaytaylor/html2text")
		}
		t.Logf("Replace version: %s", pkg.Version)
	} else {
		t.Error("Expected replace dependency github.com/jaytaylor/html2text not found")
	}

	// 验证不包含 module 和 go
	if _, ok := pkgMap["github.com/example/myapp"]; ok {
		t.Error("Should not include module declaration")
	}
	if _, ok := pkgMap["go"]; ok {
		t.Error("Should not include go version")
	}
